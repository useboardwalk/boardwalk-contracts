...
/// @title BoardwalkLPManager
/// @notice Singleton tax-exempt wrapper for adding/removing LP. The wrapper itself is in every
///         token's exempt list, so users can route LP ops through it without paying the transfer tax.
///         Restricted to pairs containing `RAISE_TOKEN` to prevent arbitrary tax-free transfers.
contract BoardwalkLPManager {
    ...
    /// @notice Tax-free addLiquidity. One of `tokenA`/`tokenB` must be `RAISE_TOKEN`.
    ...
    /// @notice Tax-free removeLiquidity. One of `tokenA`/`tokenB` must be `RAISE_TOKEN`.
    function removeLiquidity(
        ...
        // Receive into this contract first so both legs of the pair burn under our tax exemption.
