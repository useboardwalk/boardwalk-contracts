...
/// @title DeployUnsoldBurner - Deploy the unsold-BWS sink for the Uniswap CCA launch
/// @notice Deploys `UnsoldBurner(BWS)`. Set it as the CCA auction's `tokensRecipient` so unsold BWS (all of
...
contract DeployUnsoldBurner is Script {
    ...
    function run() public {
        ...
        // The burner's BWS is immutable and the auction's tokensRecipient is immutable from the
        // launch tx: a wrong token here silently strands the unsold supply, so pin the real BWS.
        if (bws.code.length == 0) revert BwsHasNoCode(bws);
        uint256 supply = IERC20(bws).totalSupply();
        ...
        UnsoldBurner burner = new UnsoldBurner(bws);
        vm.stopBroadcast();

        console.log("UnsoldBurner deployed to:", address(burner));
        console.log("  BWS token:", address(burner.BWS()));
        ...
        console.log("After the auction ends, anyone may call UnsoldBurner.sweep(auction) to pull + burn unsold BWS.");
    }
}
