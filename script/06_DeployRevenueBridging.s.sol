...
///         generic `RevenueBridger` on every non-Base LiFi lane (Ethereum, Ink, Katana, Fraxtal).
/// @dev Deploy order:
///      1. Base: run this script, which deploys `BaseRevenueSwapper`. Record its address; every other
///         lane pins it as `BASE_DESTINATION`.
///      2. Each non-Base chain: rerun with BASE_DESTINATION set to the swapper from step 1. ETH/Ink
///         deploy a pure Across V4 bridger; Katana a composed Symbiosis one; Fraxtal a composed Glacis
///         one. Fraxtal/Glacis charges a native (FRAX) fee, so the keeper holds FRAX and attaches it as
///         `msg.value` per `bridgeToBase` call (excess refunded).
...
///      7. Keep `governanceVault == address(0)` on all non-Base chains; a set vault diverts 70% past the
...
///      RAISE_TOKEN. Optional: OWNER (default deployer), MAX_FEE_BPS, RAISE_TOKEN (ETH/Ink default to
///      canonical WETH, Fraxtal to frxUSD).
