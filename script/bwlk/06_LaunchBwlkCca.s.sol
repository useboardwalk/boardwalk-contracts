...
/// @title LaunchBwlkCca - Launch the 315,000 BWLK market-formation bucket through Uniswap's CCA
/// @notice From the wallet holding the bucket, deposit + distribute through the LiquidityLauncher
///         so the LBPStrategy creates and registers the auction (sale half = 157,500 BWLK; the
///         other 157,500 is the LP reserve, paired with 100% of the ETH raised).
...
/// - CCA_REQUIRED_CURRENCY_RAISED: graduation threshold in wei (default: 0, no threshold)
/// - CCA_LIQUIDITY_LAUNCHER (default: EthereumConfig)
/// - CCA_LBP_STRATEGY (default: EthereumConfig)
/// - CCA_LAUNCH_SALT (default: 0)
contract LaunchBwlkCca is Script {
    ...
    uint128 internal constant LP_RESERVE = uint128(EthereumConfig.CCA_BUCKET - EthereumConfig.CCA_AUCTION_SUPPLY);

    struct LaunchConfig {
        address wallet; // holder of the 315,000e18 bucket; the caller of deposit+distribute
        address bwlk;
        address launcher;
        address strategy;
        address voter;
        address lpLocker; // positionRecipient: every LP position mints here
        address burner; // tokensRecipient: unsold BWLK sink
        address recipient; // leftover currency/token dust + the entire LP seed on a failed migration
        uint64 startBlock; // mainnet block numbers (the CCA's clock is block.number on chainId 1)
        uint64 endBlock;
        uint64 claimBlock;
    ...
    error BurnerBwlkMismatch(address burnerBwlk, address bwlk);
    ...
    function check(
        ...
        uint256 balance = IERC20(cfg.bwlk).balanceOf(cfg.wallet);
        if (balance < EthereumConfig.CCA_BUCKET) {
            revert WalletBucketMissing(cfg.wallet, balance, EthereumConfig.CCA_BUCKET);
        }
        ...
        if (burnerBwlk != cfg.bwlk) revert BurnerBwlkMismatch(burnerBwlk, cfg.bwlk);
        ...
        if (locker.CURRENCY0() != address(0) || locker.CURRENCY1() != cfg.bwlk) {
        ...
        // Mainnet blocks land every ~12s; leave margin for broadcast delay.
    ...
    function predictAuction(
        ...
        return ICcaAuctionFactory(factory)
            .getAddress(
                cfg.bwlk,
                EthereumConfig.CCA_AUCTION_SUPPLY, // distributed bucket minus the LP reserve
    ...
    function _execute(
        ...
        IERC20(cfg.bwlk).forceApprove(permit2, EthereumConfig.CCA_BUCKET);
        IPermit2(permit2)
            .approve(cfg.bwlk, cfg.launcher, uint160(EthereumConfig.CCA_BUCKET), uint48(block.timestamp + 1 days));
        ...
        ILiquidityLauncher.Distribution memory distribution = ILiquidityLauncher.Distribution({
            ...
            amount: uint128(EthereumConfig.CCA_BUCKET),
            configData: abi.encode(buildMigratorParameters(cfg), abi.encode(buildAuctionParameters(cfg)))
        });
        bytes[] memory calls = new bytes[](2);
        calls[0] = abi.encodeCall(ILiquidityLauncher.depositToken, (cfg.bwlk, uint160(EthereumConfig.CCA_BUCKET)));
        calls[1] = abi.encodeCall(ILiquidityLauncher.distributeToken, (cfg.bwlk, distribution, cfg.salt));
    ...
    function _configFromEnv(
        ...
        cfg.launcher = vm.envOr("CCA_LIQUIDITY_LAUNCHER", EthereumConfig.ETH_LIQUIDITY_LAUNCHER);
        cfg.strategy = vm.envOr("CCA_LBP_STRATEGY", EthereumConfig.ETH_LBP_STRATEGY);
    ...
    function _printRunbook(
        ...
        console.log("BWLK CCA launch submitted through the LiquidityLauncher.");
        console.log("");
        console.log("CCA auction (initializer):", auction);
        console.log("Record this address; every step below needs it.");
        console.log("  offered supply:", EthereumConfig.CCA_AUCTION_SUPPLY / 1e18, "BWLK");
        console.log("  LP reserve    :", uint256(LP_RESERVE) / 1e18, "BWLK (held by the strategy until migrate)");
