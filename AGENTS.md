...
- **Invariant (source chains)**: `governanceVault == address(0)` forever on ETH/Ink/Katana/Fraxtal — a set vault diverts 70% of `forwardRevenue` past the bridger. Never call `executeSetGovernanceVault` there; alert on `GovernanceVaultUpdated`. **BWS-migration carve-out (Arbitrum)**: with the BMX→BWS migration, Arbitrum becomes the protocol's governance home — at the migration cutover the Arbitrum FeeCollector's `governanceVault` is set to the BWS `GovernanceVoter` (existing 7d `executeSetGovernanceVault` timelock; the one legitimate `GovernanceVaultUpdated` event), Arbitrum leaves the source-chain set, and the revenue mesh re-homes to Arbitrum as hub (net-new deployment; the Base-hub mesh described above and Base BMX fee-direction sunset)
